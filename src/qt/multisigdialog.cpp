...
bool MultisigDialog::createMultisigTransaction(vector<CTxIn> vUserIn, vector<CTxOut> vUserOut, string& feeStringRet, string& errorRet)
{
    try{
        ...
        if(totalIn < totalOut){
            throw runtime_error("Not enough zibiz provided as input to complete transaction (including fee).");
        }
        ...
        if(tx.vout.at(changeIndex).nValue > fee){
            ...
            throw runtime_error("Not enough zibiz provided to cover fee");
        }
