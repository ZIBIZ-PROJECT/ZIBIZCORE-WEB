...
#if defined(NDEBUG)
#error "zibiz cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "zibiz very Signed Message:\n";
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from zibiz
...
bool RecalculateECASupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating zibiz supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating zibiz supply..."), percent);
        }
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert zibiz to zECA
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // zibiz: We use certain sporks during IBD, so check to see if they are
